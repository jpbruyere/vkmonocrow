...
void vke_pipeline_create (VkRenderer* r){
	...
	VkRect2D scissor = { .offset = {0, 0}, .extent = {r->width,r->height}};
	VkViewport viewport = { .width = (float) r->width,
	...
	VkPipelineShaderStageCreateInfo vertStage = {
		...
		.module = vkh_load_module(r->dev, "shaders/triangle.vert.spv"),
		.pName = "main",
	};
	VkPipelineShaderStageCreateInfo fragStage = {
		.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
		.stage = VK_SHADER_STAGE_FRAGMENT_BIT,
		.module = vkh_load_module(r->dev, "shaders/triangle.frag.spv"),
		.pName = "main",
	};
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(r->dev, &pipelineLayoutInfo, NULL, &pipelineLayout));
	...
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(r->dev, VK_NULL_HANDLE, 1, &pipelineCreateInfo, NULL, &pipeline));

	vkDestroyShaderModule(r->dev, shaderStages[0].module, NULL);
	vkDestroyShaderModule(r->dev, shaderStages[1].module, NULL);
}
